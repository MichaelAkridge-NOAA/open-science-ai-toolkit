from PyQt5.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, QFileDialog, QMessageBox, QComboBox, QCheckBox, QGroupBox, QFormLayout, QTextEdit
)
...
class YOLOTrainingGUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.device_options = {"CPU (default)": "cpu", "GPU (use GPU 0)": "0"}  # Display -> Value mapping
        ...
        self.abort_btn.setEnabled(False)  # Disabled until training starts
        self.abort_btn.clicked.connect(self.abort_training)
        layout.addWidget(self.abort_btn)

        # Scrollable Progress Text Box
        self.progress_text = QTextEdit()
        self.progress_text.setReadOnly(True)  # Make it read-only
    ...
    def start_training(self):
        selected_device_display = self.device_dropdown.currentText()
        selected_device = self.device_options[selected_device_display]
    ...
    def update_progress(self, message):
        self.progress_text.append(message)  # Append new progress to the text box
        self.progress_text.ensureCursorVisible()  # Auto-scroll to the bottom
